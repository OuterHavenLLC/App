...
class Database {
 constructor(dbName, version = 1) {
  ...
  this.db = null;
    ...
    open(storeName, keyPath = "ID", autoIncrement = false) {
        return new Promise((resolve, reject) => {
            ...
            request.onupgradeneeded = (event) => {
                ...
                if (!db.objectStoreNames.contains(storeName)) {
                    // Create store with autoIncrement option
    ...
    saveMultiple(storeName, dataArray) {
        return new Promise((resolve, reject) => {
            const transaction = this.db.transaction([storeName], 'readwrite');
            const store = transaction.objectStore(storeName);
            const results = [];
...
function AESdecrypt(Data = "") {
  var KeyWordList = CryptoJS.enc.Hex.parse(DITkey),
        Data = CryptoJS.AES.decrypt(Data, KeyWordList, {
...
function AESencrypt(Data = "") {
 var KeyWordList = CryptoJS.enc.Hex.parse(DITkey),
       ...
       Data = CryptoJS.AES.encrypt(DataWordList, KeyWordList, {
        mode: CryptoJS.mode.ECB,
        padding: CryptoJS.pad.Pkcs7
       });
...
function Card(data) {
 ...
 $("." + ID).html(Card);
 $("." + ID).fadeIn(500);
 $("." + ID).find(".CardFront").show("slide", {direction: "down"}, 500);
 setTimeout(function() {
  $(".CloseCard, .OpenCard").each(function() {
   this.disabled = false;
  });
  ...
  if($(Input).length && $(Preview).length) {
   if($(Preview).is(":visible")) {
    $.ajax({
     ...
     url: base + $.b64.d($(Input).attr("data-live-view")) + DLC
    });
   }
  }
 };
...
function ReSearch(input) {
 ...
 if(Bar === {} || typeof(Bar) === "undefined") {
  ...
  Processor = base + $.b64.d(List) + "&query=" + $.b64.e($(Bar).val());
  $(Container).empty();
  $.ajax({
   headers: {
    Language: $.b64.e(LocalData("Get", "Language")),
   ...
   success: function(data) {
    if(/<\/?[a-z][\s\S]*>/i.test(data) === true) {
     ...
     var Data = JSON.parse($.b64.d(data)),
           ...
           Grid = GridColumns,
           Response = Data.Response,
           SearchID = uniqid("SearchList");
     $(Container).html($.b64.d(Response.NoResults));
     if(AccessCode === "Accepted") {
      Extension = $.b64.d(Response.Extension);
      End = Response.End || 0;
      if(Grid === "2") {
       Grid = "Grid2";
      } else if(Grid === "3") {
       ...
       for(var i in List) {
        ...
        if(KeyCheck === 1 && ValueCheck === 1) {
         ...
         if(value !== {} && $.type(value) !== "undefined") {
          for(var i in value) {
           Result = Result.replaceAll(value[i][0], $.b64.d(value[i][1]));
  ...
  success: function(data) {
   if(/<\/?[a-z][\s\S]*>/i.test(data) === true) {
    ...
    if(AccessCode === "Denied") {
     ...
     $(DefaultContainer).html(Response);
     $(".ViewPageMainView").fadeOut(500);
     setTimeout(function() {
      $(".ParentPageMainView").show("slide", {
       direction: "left"
      }, 500);
      setTimeout(function() {
       $(".ViewPageMainView").remove();
      }, 500);
     }, 500);
     setTimeout(function() {
...
$(document).on("click", ".Attach", function() {
 var Button = $(this),
       Input = $(document).find($.b64.d($(Button).attr("data-input"))),
...
$(document).on("click", ".CloneAttachments", function() {
 var Button = $(this),
       CloneID = uniqid("AttachmentClone"),
       Destination = $(Button).attr("data-destination"),
       InjectCloneID = function(cloneID, data) {
        var link = "",
...
$(document).on("click", ".SendData", function(event) {
 var Button = $(this),
       ...
       FormData = Encode($(Form).find(Inputs).serializeArray()) || {},
       Pass = 1,
...
$(document).on("keyup", ".SearchBar", function() {
 ...
 if(getFSTvisibility() === "Accepted") {
  ...
  $.ajax({
   ...
   method: "POST",
   success: function(data) {
    $(DefaultContainer).html($.b64.d(data));
    $(".SideBar").hide("slide", {direction: "left"}, 500);
    $(".TopBar .MenuContainer").slideUp(500);
   },
